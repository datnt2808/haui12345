<template>
    <div class="admin-content">
        <div class="atable-header">
            <AdminLineLink name="Đánh giá"></AdminLineLink>
            ...
            <div class="list-name-table">Danh sách đánh giá</div>
        </div>
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th style="width: 100px">Id sản phẩm</th>
                        <th style="width: 150px">Ngày đánh giá</th>
                        <th>Thao tác</th>
                    </tr>
                </thead>
                <tbody v-for="(item, index) in dataUser" :key="index" @click="trClick(item.commentID)"
                    @dblclick="rowOnDblClick(item)" :class="{
                        'row-selected': rowSelected == item.commentID,
                    }">
                    <tr>
                        ...
                        <td>{{ item.name }}</td>
                        <td>{{ item.detail }}</td>
                        <td>Đánh giá {{ item.ratingID }} sao</td>
...
export default {
    ...
    methods: {
        questionDeleteItem(item) {
            let text = `Bạn có muốn xóa đánh giá của người dùng ${item.name} không ?`;
        ...
        deleteItem(data) {
            axios.delete(ApiUser.deleteComment(data.commentID))
                .then((res) => {
                    if (res.status == 200) {
        ...
        filterAndPaging() {
            ...
            setTimeout(() => {
                axios.get(ApiUser.filterComment(this.textSearch, this.pageSize, this.pageChoice))
                    .then((res) => {
                        if (res.status == 200) {
